...
func convertEnrollParamsFromProto(p *pb.EnrollRequest) *agent.EnrollParams {
	if p == nil {
		return nil
	}
	return &agent.EnrollParams{
		EnrollmentKey: p.EnrollmentKey,
		CsrDer:        p.CsrDer,
	}
}

func convertEnrollResultToProto(p *agent.EnrollResult) *pb.EnrollResponse {
	if p == nil {
		return nil
	}
